...
namespace PopsAnimalControl_MVC.PAC_SERVICES.Repositories.Dog_Repo
{
    public class DogRepository
    {
        private HttpClient _client;
        public DogRepository(HttpClient client)
        {
            _client = client;
        }
        public async Task<List<DogListItem>> Get(string URL, string email)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return JsonConvert.DeserializeObject<List<DogListItem>>(content);
            }
            return null;
        }
        public async Task<DogDetail> Get(string URL, string email, int id)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                return JsonConvert.DeserializeObject<DogDetail>(content);
            }
            return null;
        }
        public async Task<bool> Post(string URL, string email, DogCreate dogCreate)
        {
            ...
            if (dogCreate is null)
            {
                return false;
            }
            request.Content = new StringContent(JsonConvert.SerializeObject(dogCreate), Encoding.UTF8, "application/json");
            var response = await _client.SendAsync(request);
            if (response.IsSuccessStatusCode)
            {
                return true;
        ...
        public async Task<bool> Put(string URL, string email, DogEdit dogEdit, int id)
        {
            ...
            request.Content = new StringContent(JsonConvert.SerializeObject(dogEdit), Encoding.UTF8, "application/json");
            var response = await _client.SendAsync(request);
            if (response.IsSuccessStatusCode)
            {
                return true;
