...
namespace PopsAnimalControl_MVC.PAC_SERVICES.Repositories.CatCatcher_Repo
{
    public class CatCatcherRepository
    {
        private HttpClient _client;
        public CatCatcherRepository(HttpClient client)
        {
            _client = client;
        }
        public async Task<List<CatCatcherListItem>> Get(string URL, string email)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                return JsonConvert.DeserializeObject<List<CatCatcherListItem>>(content);
            }
            return null;
        }
        public async Task<CatCatcherDetail> Get(string URL, string email, int id)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                return JsonConvert.DeserializeObject<CatCatcherDetail>(content);
            }
            return null;
        }
        public async Task<bool> Post(string URL, string email,CatCatcherCreate catCatcher)
        {
            ...
            request.Content = new StringContent(JsonConvert.SerializeObject(catCatcher), Encoding.UTF8, "application/json");
